	...
	<div class='ewd-ufaq-submit-faq-radio-checkbox-container'>
		...
		<?php foreach ( $this->get_custom_field_options() as $option ) { ?>
			...
			<div class='ewd-ufaq-submit-faq-radio-checkbox-each'>
				<input type='radio' name='<?php echo esc_attr( $input_name ); ?>' value='<?php echo esc_attr( $option ); ?>' <?php echo ( ! empty( $_POST[ $input_name ] ) and $option == $_POST[ $input_name ] ? 'checked' : '' ); ?> <?php echo $this->get_field_required(); ?>/><?php echo esc_html( $option ); ?>
			</div>
